...
/*  05-01-2016     Yuxin Zhou               Modified comment(s), and      */
...
/* A DHCP client must be prepared to receive DHCP messasgs with an 'options' field of 
...
/* Define the timing and retry constants for ARP probe. RFC5227, Section1.1, Page5.  */
...
/*  Define the wait time before restarting the configuration process when detects that the address is already in use. 
    The client SHOULD wait a minimum of ten seconds before restarting the configuration process
    to avoid excessive network traffic in case of looping. RFC2131, Section3.1, Page17.  */
...
typedef struct NX_DHCP_STRUCT 
{
    ...
    ULONG           nx_dhcp_network_mask;       /* Server assigned network mask                             */  
    ULONG           nx_dhcp_gateway_address;    /* Server assigned gateway address                          */ 
    UCHAR           nx_dhcp_options_buffer[NX_DHCP_OPTIONS_BUFFER_SIZE]; /* Store DHCP options.             */
