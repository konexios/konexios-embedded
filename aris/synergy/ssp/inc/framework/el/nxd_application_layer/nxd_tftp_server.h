...
/* Define the number of retries if the TFTP server resends data or an ACK packet if it
   receives a previous (duplicate) ACK or data packet from the TFTP Client. This does 
   not utilize the retransmission timeout and is available only
   if NX_TFTP_SERVER_RETRANSMIT_ENABLE is not defined.  */
