...
DSTATUS TM_FATFS_SD_disk_initialize(BYTE lun);
DSTATUS TM_FATFS_SD_disk_status(BYTE lun);
DRESULT TM_FATFS_SD_disk_read(BYTE lun, BYTE *buff, DWORD sector, UINT count);
DRESULT TM_FATFS_SD_disk_write(BYTE lun, const BYTE *buff, DWORD sector, UINT count);
DRESULT TM_FATFS_SD_disk_ioctl(BYTE lun, BYTE cmd, void *buff);
...
#else
	TM_SPI_WriteMulti(FATFS_SPI, (BYTE *)buff, btx);
...
)
{
	...
	case CTRL_ERASE_SECTOR :	/* Erase a block of sectors (used when _USE_ERASE == 1) */
		...
		if (TM_FATFS_SD_disk_ioctl(0, MMC_GET_CSD, csd)) break;	/* Get CSD */
